"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ConfigService = exports.BaseConfigService = void 0;
const common_1 = require("@nestjs/common");
const config_types_1 = require("./config.types");
let BaseConfigService = class BaseConfigService {
    constructor(env) {
        this.env = env;
    }
    isDevelopment() {
        return this.valueFor(ConfigService.ConfigVar.appEnv, '') === 'development';
    }
    valueFor(configVar, defaultValue) {
        const value = this.env[configVar];
        if (value) {
            return value;
        }
        return defaultValue || '';
    }
};
BaseConfigService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)(config_types_1.PROCESS_ENV)),
    __metadata("design:paramtypes", [Object])
], BaseConfigService);
exports.BaseConfigService = BaseConfigService;
let ConfigService = class ConfigService extends BaseConfigService {
};
ConfigService = __decorate([
    (0, common_1.Injectable)()
], ConfigService);
exports.ConfigService = ConfigService;
(function (ConfigService) {
    let ConfigVar;
    (function (ConfigVar) {
        ConfigVar["appEnv"] = "APP_ENV";
        ConfigVar["logging"] = "LOGGING";
        ConfigVar["loggingLevel"] = "LOGGING_LEVEL";
        ConfigVar["loggingVerbosity"] = "LOGGING_VERBOSITY";
    })(ConfigVar = ConfigService.ConfigVar || (ConfigService.ConfigVar = {}));
})(ConfigService = exports.ConfigService || (exports.ConfigService = {}));
exports.ConfigService = ConfigService;
//# sourceMappingURL=base-config.service.js.map